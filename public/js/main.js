...
$('#country').on('change', function (e) {
    if($('select[id=country]').val() != "0") {
        ...
        $("#state option[value='0']").text('Please wait..');
$.ajaxSetup({
  headers: {
...
$('#state').on('change', function (e) {
    if($('select[id=country]').val() != "0") {
        ...
        $("#city option[value='0']").text('Please wait..');
$.ajaxSetup({
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  }
        ...
        _createAutocomplete: function() {
            ...
            this.input = $("<input>").appendTo(this.wrapper).val(i).attr("title", "").addClass("custom-combobox-input ui-widget ui-widget-content ui-state-default ui-corner-left lp-search-input location_input lp-home-locaton-input").autocomplete({
                ...
                select: function(event, ui) {
                    if($( this ).hasClass( "city-combo" )) {
                    $(".city-sel").val(ui.item.option.value);
                    }
...
$('.your-city option[value!="1"] ').remove();
$.ajaxSetup({
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  }
...
$.ajax({
            ...
            var classList = $(this.element.parent().children().first()).attr('class');
            var athis = this;
            if(classList) {
              classList =  classList.split(/\s+/);
$.each(classList, function(index, item) {
    if (item === 'your-city') {
        athis.input.addClass('city-combo');
    }
    if (item === 'your-state') {
